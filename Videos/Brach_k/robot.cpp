...
int  ROBOT::Has_Stopped(void) {
	...
	return( neuralNetwork->Sensors_No_Longer_Changing() );
}
...
void ROBOT::Label(NEURAL_NETWORK *genome) {
...
void ROBOT::Neural_Network_Set_Sensor_Neurons(void) {

	int sensorIndex = 0;

	Neural_Network_Set_Sensor_Neurons_In_Objects(&sensorIndex);

	Neural_Network_Set_Sensor_Neurons_In_Joints(&sensorIndex);
}

void ROBOT::Neural_Network_Set_Sensor_Neurons_In_Joints(int *sensorIndex) {
	...
	for (int j=0;	j<numJoints;	j++) {
		...
		if ( joints[j] ) {
			...
			if ( joints[j]->proprioceptiveSensor ) {
				...
				neuralNetwork->Set_Sensor_Neuron(
					(*sensorIndex),
					sensorValue);
...
void ROBOT::Neural_Network_Set_Sensor_Neurons_In_Objects(int *sensorIndex) {
	...
	for (int i=0;	i<numObjects;	i++) {
		...
		if ( objects[i] ) {
			...
			if ( objects[i]->touchSensor ) {
				...
				double sensorValue = 
				objects[i]->touchSensor->Get_Value();
...
void ROBOT::Neural_Network_Update(void) {

	if ( !neuralNetwork )
		return;

	Neural_Network_Set_Sensor_Neurons();
