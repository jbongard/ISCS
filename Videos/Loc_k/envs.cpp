...
void ENVS::Evolve(	dWorldID      world, 
	...
	if ( !Eval_Finished() ) {
		...
		for (int i=0;	i<numberOfEnvs;	i++) {
			...
			//if ( !taskEnvironments[i]->Robot_Has_Stopped(optimizer->timer) )

				taskEnvironments[i]->Allow_Robot_To_Move();
		}
	...
	// and prepare for the evaluation of the next robot.
		...
		for (int i=0;i<numberOfEnvs;i++) {
			double currFit = taskEnvironments[i]->Fitness_Get();
			if ( currFit < fitness )
				fitness = currFit;
		}
...
void ENVS::Create_Robot_Current_Best( 	dWorldID      world, 
	...
	for (int i=0;	i<numberOfEnvs;	i++) {
		...
		taskEnvironments[i]->Label(optimizer->Genome_Get_Best());
	}
}
...
int  ENVS::Eval_Finished(void) {
	...
	return( Robots_Have_Stopped() );
//	return( A_Robot_Has_Stopped() );
}
