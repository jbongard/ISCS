...
void ENVS::Create_Robot_Current_Best( 	dWorldID      world, 
	...
	for (int i=0;	i<numberOfEnvs;	i++) {
		...
		taskEnvironments[i]->Label(optimizer->Genome_Get_Best(),i);
	}
}
...
void ENVS::Create_Robot_To_Evaluate(	dWorldID      world, 
	...
	for (int i=0;	i<numberOfEnvs;	i++) {
		...
		if ( i==0 )
			taskEnvironments[i]->Label(optimizer->Genome_Get_Next_To_Evaluate(),
						   i);
		else
			taskEnvironments[i]->Label(optimizer->Genome_Get_Curr_To_Evaluate(),
						   i);
	}
}
...
int  ENVS::Eval_Finished(void) {
	...
	if (	In_Evolution_Mode() &&
		...
		Robots_Are_In_A_Fixed_Attractor() ) {
...
*/
	...
	if (	In_Evolution_Mode() &&
		...
		optimizer->Current_Robot_Failed(Fitness_Get()) )
...
double ENVS::Fitness_Get(void) {
	...
	if ( Robots_Are_In_A_Fixed_Attractor() )
...
int  ENVS::Robots_Are_In_A_Fixed_Attractor(void) {
	...
	int inAFixedAttractor = true;
	int i = 0;

	while (	(inAFixedAttractor) &&

		(i<numberOfEnvs) ) {

		inAFixedAttractor = taskEnvironments[i]->Robot_In_A_Fixed_Attractor();

		if ( inAFixedAttractor )

			i++;
	}

	return( inAFixedAttractor );
}
