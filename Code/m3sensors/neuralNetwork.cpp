...
/*
	inAFixedAttractor = true;
	int j=0;

	while ( (inAFixedAttractor) && (j<numNodes) ) {

		if ( temp->vals[j+0] != nodeValues->vals[j+0] )

			inAFixedAttractor = false;

		if ( temp->vals[j+1] != nodeValues->vals[j+1] )
...
*/
	...
	for (int j=0;	j<numNodes;	j++) {
		...
		// Update the two motor and hidden nodes,
