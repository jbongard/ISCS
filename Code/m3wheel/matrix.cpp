...
MATRIX::MATRIX(int ln, int wd, double min, double max) {
...
int  MATRIX::ColumnValuesEqual(int j) {

	int i=0;
	int equal = true;

	while ( (equal) && (i<(length-1)) ) {
...
int    MATRIX::Equals(MATRIX *other) {
...
void MATRIX::FlipRandomBit(void) {
	...
	Flip(RandInt(0,length-1),RandInt(0,width-1));
}

void MATRIX::FlipRandomBitInRow(int i) {

	Flip(i,RandInt(0,width-1));
}
...
void MATRIX::Mutate(double mutationProbability, double min, double max) {
...
//			if ( Rand(0,1) < mutationProbability ) {
				...
				double randN = mutationProbability * (max-min) * RandN();
				...
				//Set(i,j, Rand(min,max) );
//			}
		}
}

int  MATRIX::No_Value_Above(double val) {

	int noValueAbove = true;

	int i=0;

	while ( (i<length) && (noValueAbove) ) {

		int j=0;

		while ( (j<width) && (noValueAbove) ) {
...
void MATRIX::RoundToDecimalPlace(int howMany) {

	for (int i=0;i<length;i++)

		for (int j=0;j<width;j++)

			RoundToDecimalPlace(i,j,howMany);
}

void MATRIX::RoundToDecimalPlace(int i, int j, int howMany) {
...
double MATRIX::Rand(double min, double max) {
