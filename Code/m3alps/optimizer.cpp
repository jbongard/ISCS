...
void OPTIMIZER::Fitness_Receive(double fitness) {
	...
	if ( genomeUnderEvaluation ) {
		...
		Genome_Get_Next_To_Evaluate()->Fitness_Set(fitness);
...
NEURAL_NETWORK *OPTIMIZER::Genome_Get_Best(void) {
	...
	for (int i=0;	i<populationSize;	i++)
		...
		if ( genomes[i]->Fitness_Get() > bestFit ) {

			bestFit   = genomes[i]->Fitness_Get();
			bestIndex = i;
		}
...
void OPTIMIZER::Destroy_Worst_Fitnesses(void) {

	delete worstFitnesses;
	worstFitnesses = NULL;
}
...
void OPTIMIZER::Genomes_Print(void) {
	...
	for (int i=(ALPS_NUM_LAYERS-1);	i>=0;	i--) {
		...
		printf("Layer: %d \t	Age cap: %d \t",i,Layer_Age_Cap(i) );
...
*/
	...
	for (int j=0;	j<ALPS_NUM_LAYERS;	j++)
		...
		ageCaps->Set(0,j, int(floor(1.2*(10*j))) );
}

void OPTIMIZER::Initialize_Worst_Fitnesses(void) {

	worstFitnesses = new MATRIX(1,ALPS_NUM_LAYERS);

	for (int j=0;	j<ALPS_NUM_LAYERS;	j++)

		worstFitnesses->Set(0,j,WORST_FITNESS);
}
...
double OPTIMIZER::Layer_Worst_Fitness(int layerIndex) {

	double worstFitness = 1000.0;

	for (int j=0;	j<ALPS_GENOMES_PER_LAYER;	j++)

		if (	(Genome_Get(layerIndex,j)->Evaluated()) &&

			(Genome_Get(layerIndex,j)->Fitness_Get() < worstFitness) )

			worstFitness = Genome_Get(layerIndex,j)->Fitness_Get();
...
void OPTIMIZER::TryMoveUp(int layerIndex) {

	int victimIndex = Layer_Find_Victim(layerIndex,genomeUnderEvaluation->Fitness_Get());
	...
	if ( victimIndex > -1 ) {
		...
		if ( Layer_Top(layerIndex) ) {
			...
			temp = NULL;
			TryMoveUp(layerIndex+1);
		}

		Layer_Sort(layerIndex);
	}
	else {
		delete genomeUnderEvaluation;
		genomeUnderEvaluation = NULL;
