...
extern double	ROBOT_WHEEL_BODY_RADIUS;
extern double	ROBOT_WHEEL_LEG_LENGTH;
extern int	ROBOT_WHEEL_LEG_SEGS;
extern double	ROBOT_WHEEL_LEG_RADIUS;
extern int	ROBOT_WHEEL_LEGS;
extern double	ROBOT_WHEEL_FOOT_LENGTH;
extern double	ROBOT_WHEEL_FOOT_WIDTH;
extern double	ROBOT_WHEEL_FOOT_HEIGHT;
...
void ROBOT::Create_Wheel(void) {

	Create_Wheel_Objects();

	Create_Wheel_Joints();
}

void ROBOT::Create_Wheel_Add_Foot(      double x, double y, double z,
					double angle,
					int *currObject) {
	...
	double yCenter = y + 0.5 * ROBOT_WHEEL_FOOT_LENGTH * sin( DegToRad(angle) );
	double zCenter = z + 0.5 * ROBOT_WHEEL_FOOT_LENGTH * cos( DegToRad(angle) );

	double yEnd    = y + 1.0 * ROBOT_WHEEL_FOOT_LENGTH * sin( DegToRad(angle) );
	double zEnd    = z + 1.0 * ROBOT_WHEEL_FOOT_LENGTH * cos( DegToRad(angle) );

/*
  	objects[*currObject] = new OBJECT(SHAPE_RECTANGLE,

				ROBOT_WHEEL_FOOT_HEIGHT,
				ROBOT_WHEEL_FOOT_HEIGHT,
				ROBOT_WHEEL_FOOT_LENGTH,
				x,
				yCenter,
				zCenter,
				0,
				...
				z-zCenter);
*/
  	objects[*currObject] = new OBJECT(SHAPE_CYLINDER,

				ROBOT_WHEEL_FOOT_HEIGHT/2.0,
				ROBOT_WHEEL_FOOT_LENGTH,
				x,
				yCenter,
				zCenter,
	...
	objects[*currObject]->Sensor_Touch_Add();

	(*currObject) = (*currObject) + 1;	
}

void ROBOT::Create_Wheel_Add_Foot_Joint(	double x, double y, double z,
					double angle,
					int *currObject) {

	joints[(*currObject)-1] = new JOINT(this,(*currObject)-1,*currObject,
				x,y,z,
				1,0,0,
				-150,
				+150);
...
void ROBOT::Create_Wheel_Add_Joint(	double x, double y, double z,
					double angle, int segIndex,
					int *currObject, int legBase) {

	double maxFlexion   = -ROBOT_STARFISH_JOINT_RANGE;
	double maxExtension = +ROBOT_STARFISH_JOINT_RANGE;

	if ( legBase )

		joints[(*currObject)-1] = new JOINT(this,0,*currObject,
					x,y,z,
					1,0,0,
					maxFlexion,
					maxExtension);
	...
	double yCenter = y + 0.5 * ROBOT_WHEEL_LEG_LENGTH * sin( DegToRad(angle) );
	double zCenter = z + 0.5 * ROBOT_WHEEL_LEG_LENGTH * cos( DegToRad(angle) );

	double yEnd    = y + 1.0 * ROBOT_WHEEL_LEG_LENGTH * sin( DegToRad(angle) );
	double zEnd    = z + 1.0 * ROBOT_WHEEL_LEG_LENGTH * cos( DegToRad(angle) );

	if ( segIndex>0 )

		Create_Wheel_Add_Joint(     x,yEnd,zEnd,angle,segIndex-1,&(*currObject),false);

	else

		Create_Wheel_Add_Foot_Joint(x,yEnd,zEnd,angle,&(*currObject));
}

void ROBOT::Create_Wheel_Add_Leg_Seg(	double x, double y, double z,
					double angle, int segIndex,
					int *currObject, int legBase) {
	...
	double zEnd    = z + 1.0 * ROBOT_WHEEL_LEG_LENGTH * cos( DegToRad(angle) );
  	...
  	objects[*currObject] = new OBJECT(SHAPE_CYLINDER, 
				ROBOT_WHEEL_LEG_RADIUS,
				ROBOT_WHEEL_LEG_LENGTH,
				x,
				yCenter,
				zCenter,
				0,
	...
	objects[*currObject]->Sensor_Light_Add();

	(*currObject) = (*currObject) + 1;

	if ( segIndex>0 )

		Create_Wheel_Add_Leg_Seg(x,yEnd,zEnd,angle,segIndex-1,&(*currObject),false);

	else

		Create_Wheel_Add_Foot   (x,yEnd,zEnd,angle,&(*currObject));
}

void ROBOT::Create_Wheel_Joints(void) {

	numJoints = ROBOT_WHEEL_LEGS * (ROBOT_WHEEL_LEG_SEGS+1);
	...
	double x = 0.0;
	double y = 0.0;
	double z = ROBOT_WHEEL_FOOT_HEIGHT/2.0 + 
		   ROBOT_WHEEL_LEG_SEGS * ROBOT_WHEEL_LEG_LENGTH;

	double angle = 90.0;

	int currObject = 1;
	for (int i=0;	i<ROBOT_WHEEL_LEGS;	i++) {

		Create_Wheel_Add_Joint(x,y,z,angle,ROBOT_WHEEL_LEG_SEGS-1,&currObject,true);

		angle = angle + (360/ROBOT_WHEEL_LEGS)/2.0;

	}
}

void ROBOT::Create_Wheel_Objects(void) {

	numObjects = 1 + ROBOT_WHEEL_LEGS * (ROBOT_WHEEL_LEG_SEGS+1);
	...
	for (int i=0;	i<ROBOT_WHEEL_LEGS;	i++) {
		...
		Create_Wheel_Add_Leg_Seg(x,y,z,angle,ROBOT_WHEEL_LEG_SEGS-1,&currObject,true);
